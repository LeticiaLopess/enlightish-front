...
export function ButtonSecondary({ children, icon, variant = 'night', iconPosition = 'left', iconVariant = 'bold', className, onClick }: ButtonSecondaryProps) {
    ...
    return (
        <button
            onClick={onClick}
            className={`w-fit py-2 px-5 rounded-lg flex justify-center items-center gap-2 ${variant === 'day' ? 'bg-day-300 hover:bg-day-300/95 text-day-950' : 'bg-night-50 hover:bg-night-50/95 text-night-400'} hover:shadow-sm ${className}`}
        >
